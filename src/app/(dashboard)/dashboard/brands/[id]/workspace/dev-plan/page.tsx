    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--bm-color-text-secondary)]">
            Corte actual basado en dev plan versus implementación real.
          </p>
        </div>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        activeStep === idx
                          ...
                          : "text-[var(--bm-color-text-secondary)] hover:bg-[var(--bm-color-surface)] hover:text-[var(--bm-color-text-primary)]"
                      )}
                    >
                      <span>{step.name}</span>
                      <span className="text-xs text-[var(--bm-color-text-tertiary)]">
                ...
                <div className="space-y-1">
                  {phases.map((phase, idx) => (
                    ...
                    >
                      ...
                      <p className="mt-0.5 text-xs text-[var(--bm-color-text-tertiary)]">
                        {phase.title}
                      </p>
                    </button>
                  ))}
              ...
              <div className="rounded-xl border border-[var(--bm-color-border)] bg-[var(--bm-color-surface-muted)] p-5">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {currentPhase?.modules.map((module) => (
                    ...
                    >
                      <h4 className="text-sm font-medium text-[var(--bm-color-text-primary)]">
                        {module.name}
                      </h4>
                      ...
                      <div className="mt-3 flex items-center justify-between text-[11px] text-[var(--bm-color-text-secondary)]">
                        ...
                        <span className="font-mono text-[var(--bm-color-text-tertiary)]">
                          {module.key}
                        </span>
                      </div>
                    </article>
      ...
      <div className="space-y-3">
        {planPhases.map((phase) => (
          <article
            key={phase.id}
            className="rounded-xl border border-[var(--bm-color-border)] bg-[var(--bm-color-surface-muted)] p-4"
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.18em] text-[var(--bm-color-text-tertiary)]">
                  Fase {phase.id}
                </p>
                <h3 className="text-base text-[var(--bm-color-text-primary)]">{phase.title}</h3>
              </div>
                  ...
                  className={cn(
                    ...
                    phase.status === "done"
                      ...
                      : phase.status === "in_progress"
                        ? "text-yellow-400"
                        : "text-[var(--bm-color-text-secondary)]"
                  )}
                >
                  {phase.status.replace("_", " ")}
                </p>
                <p className="text-sm text-[var(--bm-color-text-primary)]">{phase.progress}</p>
              </div>
            </div>
          </article>
        ))}
