...
import { buildManualPdfBuffer } from "@/lib/exporters/pdf-generator";
...
export async function POST(request: NextRequest) {
  ...
  const pdf = buildManualPdfBuffer(brand.name, modules || []);

  return new Response(pdf, {
    headers: {
      "content-type": "application/pdf",
      "content-disposition": `attachment; filename=\"${brand.name.replace(/\s+/g, "-")}-manual.pdf\"`,
    },
  });
}
